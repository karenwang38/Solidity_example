...
contract("ERC20Token", function(accounts) {
  ...
  it("1. check initial value : token name, number, symbol and creator.", function() {
    return ERC20Token.deployed().then(function(instance) {
      ERC20TokenPara = instance;
      return ERC20TokenPara.name();
    }).then(function(Name) {
      assert.equal(Name, "KW", "token name is KW.");


      return ERC20TokenPara.totalSupply();
    }).then(function(Num) {
      assert.equal(Num.s, 1, "token number is 1000");
      assert.equal(Num.e, 21, "token number is 1000");
      assert.equal(Num.c, 10000000, "token number is 1000");


      return ERC20TokenPara.owner();
    }).then(function(Owner) {
      assert.equal(Owner, accounts[0], "token creator is accounts[0].");

      return ERC20TokenPara.symbol();
    }).then(function(Symbol) {
      assert.equal(Symbol, "0_0", "token symbol is 0_0.");

      return ERC20TokenPara.tokenowner(0);
      ...
      assert.equal(TokenList[1].s, 1, "first token accounts[0] balance is 1000.");
      assert.equal(TokenList[1].e, 21, "first token accounts[0] balance is 1000.");
      assert.equal(TokenList[1].c, 10000000, "first token accounts[0] balance is 1000.");
    });
  });
